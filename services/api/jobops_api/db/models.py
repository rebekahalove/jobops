...
class JobListingSource(Base, TimestampMixin):
    ...
    application_fields_json: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)
    application_requirements_json: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)
    pay_transparency_json: Mapped[dict[str, Any] | None] = mapped_column(JSON, nullable=True)
    first_seen_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    last_seen_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    last_synced_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    source_updated_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
