...
export async function GET(request: Request, context: RouteContext) {
  ...
  const url = new URL(`${config.apiBaseUrl.replace(/\/$/, "")}/v1/companies/${encodeURIComponent(companyId)}`);
  ...
  try {
    ...
    return NextResponse.json(
      {
        ok: false,
        error: "Company detail API is unavailable. Start the FastAPI service on JOBOPS_API_BASE_URL."
      },
      { status: 503 }
    );
  }
