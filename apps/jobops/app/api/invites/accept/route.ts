...
export async function POST(request: Request) {
  ...
  if (typeof username !== "string" || !username.trim() || typeof displayName !== "string" || !displayName.trim() || typeof password !== "string" || !password) {
  ...
  const apiResponse = await fetch(`${config.apiBaseUrl.replace(/\/$/, "")}/v1/auth/invites/accept`, {
    method: "POST",
    headers: {
