...
export async function PATCH(
  request: Request,
  { params }: { params: Promise<{ fieldGroup: string; fieldName: string }> }
) {
  const { fieldGroup, fieldName } = await params;
  ...
  try {
    const apiResponse = await fetch(
      `${config.apiBaseUrl.replace(/\/$/, "")}/v1/profile/fields/${encodeURIComponent(fieldGroup)}/${encodeURIComponent(fieldName)}`,
      {
