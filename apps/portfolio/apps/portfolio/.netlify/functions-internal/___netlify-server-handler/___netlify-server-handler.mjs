...
} from '/var/task/apps/portfolio/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer, withActiveSpan } from '/var/task/apps/portfolio/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('/var/task/apps/portfolio')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return withActiveSpan(tracer, 'Next.js Server Handler', async (span) => {
      span?.setAttributes({
        ...
        monorepo: true,
        cwd: '/var/task/apps/portfolio',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('/var/task/apps/portfolio/.netlify/dist/run/handlers/server.js')
